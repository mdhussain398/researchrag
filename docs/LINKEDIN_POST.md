...
Instead of building another chatbot, I engineered a structured research-analysis product that ingests multiple papers and automatically produces publication-grade synthesis reports with strict citation grounding.
...
2. **Dense Vector Search with FAISS**: Chunks are embedded into 384-dimensional dense vectors using `all-MiniLM-L6-v2` with exact $L_2$-normalized cosine similarity.
...
5. **Automated Citation Grounding Audit**: Every claim is cited `[N]` and verified against underlying chunk text and page numbers using token overlap, achieving an **80.5% Semantic Grounding Score** and **96.9% Retrieval Precision@K** on our benchmark suite!
